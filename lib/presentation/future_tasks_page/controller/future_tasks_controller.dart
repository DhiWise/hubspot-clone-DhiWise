...
import 'package:hbs/presentation/future_tasks_page/models/future_tasks_model.dart';
...
class FutureTasksController extends GetxController {
  FutureTasksController(this.futureTasksModelObj);

  Rx<FutureTasksModel> futureTasksModelObj;
  ...
  void onReady() {
    ...
    endDateFilter.koperator = "GT";
    endDateFilter.value = endDate;
    filterGroups.filters?.add(endDateFilter);

    postTasksSearchReq.limit = 50;
    postTasksSearchReq.after = 0;
    postTasksSearchReq.properties = ["hs_task_body","hs_task_priority","hs_task_status","hs_task_type","hs_timestamp","hs_task_subject"];
  ...
  onSelected(dynamic value) {
    ...
    futureTasksModelObj.value.dropdownItemList.forEach((element) {
    ...
    Filters taskTypeFilters = Filters();

    if (taskType.isNotEmpty) {
      taskTypeFilters.propertyName = "hs_task_type";
      taskTypeFilters.koperator = "EQ";
      taskTypeFilters.value = taskType;
      filterGroups.filters?.add(taskTypeFilters);
    }
    ...
    futureTasksModelObj.value.dropdownItemList.refresh();
  }

  onSelected1(dynamic value) {
    selectedDropDownValue1 = value as SelectionPopupModel;
    futureTasksModelObj.value.dropdownItemList1.forEach((element) {
      element.isSelected = false;
      if (element.id == value.id) {
        element.isSelected = true;
      }
    ...
    var taskType = "";
    if (selectedDropDownValue?.id == 2) {
      taskType = "CALL";
    } else if (selectedDropDownValue?.id == 3) {
      taskType = "EMAIL";
    } else if (selectedDropDownValue?.id == 4) {
      taskType = "TODO";
    } else {
      taskType = "";
    }
    ...
    futureTasksModelObj.value.dropdownItemList1.refresh();
  }
  ...
  Future<void> _onCreateTasksSearchSuccess() async {
    ...
    List<FutureTasksItemModel> futureTasksItemModelList = [];
    if (postTasksSearchResp!.results! != null &&
        postTasksSearchResp!.results!.isNotEmpty) {
      for (var element in postTasksSearchResp!.results!) {
        var futureTasksItemModel = FutureTasksItemModel();
        futureTasksItemModel.callTxt.value =
            element.properties!.hsTaskSubject!.toString();
        futureTasksItemModel.taskStatusImg.value =
        element.properties!.hsTaskStatus!.toString() == "COMPLETED" ? ImageConstant.imgCheckmark30X30 :  ImageConstant.imgCheckmark28X28;
        futureTasksItemModel.textTxt.value =
            element.properties!.hsTaskType!.toString();
        futureTasksItemModel
            .timeTxt.value = element.properties!.hsTimestamp!.toString();
        var contactId = associatedContacts[ element.id];
        futureTasksItemModel.taskContact.value = contactId != null
            ? contactNames[contactId.toString()]
            : "unassociated";
        futureTasksItemModelList.add(futureTasksItemModel);
      }
    }
    futureTasksModelObj.value.futureTasksItemList.value =
        futureTasksItemModelList;

    futureTasksModelObj.value.taskCounterTxt.value =
